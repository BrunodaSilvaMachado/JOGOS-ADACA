...
//imagem 2
    vet[1][0].draggable({
        ...
        stop: function() {
            i=1;
            mouseDrag += mousex+","+mousey+")";
            if (vet[i-1][5] == 2 && mousex > vet[i][9].offset().left && mousex < (vet[i][9].offset().left + vet[i][3]) && mousey > vet[i][9].offset().top && mousey < (vet[i][9].offset().top + vet[i][4] + 100)){
				$("#container").append(vet[i][0]);
                vet[i][0].css({left: vet[i][9].offset().left, top: vet[i][9].offset().top});
				vet[i][5] = 2;
                vet[i][0].draggable('disable');
...
//imagem 4
    ...
    function ajaxSave() {
        ...
        var idAtividade = 37;    //alterar esse id para o número do jogo
