...
$(document).ready(function() {
   ...
   function rand(num_minimo,num_maximo) {
    ...
    var musicaFundo = getQueryVariable("campoEscondidoMusicaFundo");                    // retorn 0-sem musica      
      ...
      /* apenas para testes
    var tipodeDica ="Pisca";                                  //"Seta" ou "Pisca"
    ...
    */
    ...
    var numObjtos = 6;     //num fixo de quantidados de objetos presentes no jogo
    var numTotal = 6;        //num de imagens/targets totais

    //var auxiliares para fazer a randomização
    var aux, j, jj;
    jj=0;
...
//  [ID][0-imagem, 1-Xinicial, 2-Yinicial, 3-larguraX, 4-larguraY,
    ...
    vet = new Array(
        new Array($("#img"+sortido[0]), 20, 50, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[0])),
        new Array($("#img"+sortido[1]), 20, 270, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[1])),
        new Array($("#img"+sortido[2]), 20, 490, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[2])),
        new Array($("#img"+sortido[3]), 210, 50, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[3])),
        new Array($("#img"+sortido[4]), 210, 270, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[4])),
        new Array($("#img"+sortido[5]), 210, 490, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[5]))
    );
    
        //reposicionamento
    vet[0][0].css({left: 20, top: 50});
    vet[1][0].css({left: 20, top: 270});
    vet[2][0].css({left: 20, top: 490});
    vet[3][0].css({left: 210, top: 50});
    vet[4][0].css({left: 210, top: 270});
    vet[5][0].css({left: 210, top: 490});
...
//Função de Redimensionamento
   /* var atividadeInterval;
    var baseSize;
    function startAtividadeInterval(){
        atividadeInterval = setInterval(function(){
            baseSize = $(window).width()<$(window).height()?$(window).width():$(window).height();
            CP = baseSize/744;
            $('body').css("zoom", (CP));
        },100);
    }
    
    function stopAtividadeInterval(){
        clearInterval(atividadeInterval);
        ////console.log("Cleared");
    }
    startAtividadeInterval();*/
...
//imagem 6
    ...
    function ajaxSave() {
        ...
        var idAtividade = 46;    //alterar esse id para o número do jogo
