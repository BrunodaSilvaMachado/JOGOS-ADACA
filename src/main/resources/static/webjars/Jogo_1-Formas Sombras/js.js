var urlGlobal = window.location.search.substring(1);
function getQueryVariable(variable)
{
    var query = window.location.search.substring(1);
    var vars = query.split("%26");
    for (var i=0;i<vars.length;i++) {
        var pair = vars[i].split("%3D");
        if(pair[0] == variable){return pair[1];}
    }
    return(false);
}

$(document).ready(function() {
    ////console.log(urlGlobal);

    var finalizado = 0;

    function rand(num_minimo,num_maximo) {
        return Math.floor((Math.random() * (num_maximo-num_minimo+1))+num_minimo);
    }

    var sort1=  rand(1,3);
    var sort2 =  sort1;
    var sort3 =  sort1;

    while( sort2 == sort1 || sort3 == sort1 || sort3 == sort2) {
        sort2 = rand(1,3);
        sort3= rand(1,3);
    };


    var aux1;
    var aux2; 
    var aux3;

    // aqui estou pegando os valores passados na url de cada jogo
    var tipodeDica = getQueryVariable("DicaPiscaOuSeta");                                  //"Seta" ou "Pisca"
    var DicaTempoSouN = getQueryVariable("campoEscondidoDicaTempo");                       //true ou false
    var tempodeDica = parseInt(getQueryVariable("campoEscondidoTempodeDica"))*1000;        //numero
    var DicaCaminhoSouN = getQueryVariable("campoEscondidoDicaCaminho");                   //true ou false
    var comemoracao = getQueryVariable("campoEscondidoComemoracao");                    //retorna 0-sem comemoração, 1-parabens animado, 2-parabens parado
    var tipoSom = getQueryVariable("campoEscondidoSom");                                 //retorna 0-sem som, 1- palmas
    var musicaFundo = getQueryVariable("campoEscondidoMusicaFundo");                     // retorn 0-sem musica

    //sintaxe:
    //  [ID][0-imagem, 1-Xinicial, 2-Yinicial, 3-larguraX, 4-larguraY,
    //   5-state, 6-tipoDica, 7-tempoDica, 8-interval, 9-target]
    vet = new Array(
        new Array($("#img"+sort1), 30, 30, 200, 200, 0, tipodeDica, tempodeDica, 0, $("#target"+sort1)),
        new Array($("#img"+sort2), 30, 265, 200, 200, 0, tipodeDica, tempodeDica, 0, $("#target"+sort2)),
        new Array($("#img"+sort3), 30, 500, 200, 200, 0, tipodeDica, tempodeDica, 0, $("#target"+sort3))
    );
    ////console.log(vet[0][0]);



    //Embaralhamento das imagens
    if (sort1 == 1 && sort2 ==3) {
        aux1=0;
        aux1 = vet[1][2];
        aux2 = vet[2][2]
        vet[1][0].css({top: aux1});
        vet[2][0].css({top: aux2});
    } else if (sort1==3 && sort2==1) {
        aux1=0;
        aux2=0;
        aux1 = vet[1][2];
        vet[0][0].css({top: vet[0][2]});
        aux2 = vet[2][2];
        vet[1][0].css({top: aux1});
        vet[2][0].css({top: aux2});
    } else if (sort3== 3 && sort2==1){
        aux = vet[0][2];
        vet[1][0].css({top: vet[1][2]});
        vet[0][0].css({top: vet[0][2]});
    } else if( sort2 == 2 && sort3 == 1){
        vet[0][0].css({top: vet[0][2]});
        vet[2][0].css({top: vet[2][2]});   
    } else if (sort1== 2 && sort2 == 3) {
        vet[2][0].css({top: vet[2][2]});
        vet[0][0].css({top: vet[0][2]});
        vet[1][0].css({top: vet[1][2]});
    }

    //Criação de Variáveis

    var numObjtos = 3; //quantidade de figuras
    var CP=1;     //

    var quantDicasUsadas = 0;
    var cliqueCerto = 0;
    var mouseDrag = "";
    var mouseClique = "";
    var cliqueErrado = 0;
    var posErrado = "";

    var fundo = $("#fundo");

    var parabens = $("#parabens"); //Variável para acessar o parabéns

    var dica = $("#dica"); // Variável para acessar a dica


    //Pega tempo do jogo
    //var hora_inicio = ((new Date().getHours()<10)?("0"+new Date().getHours()): new Date().getHours())+ ":" + ((new Date().getMinutes()<10)?("0"+new Date().getMinutes()): new Date().getMinutes()) + ":" + ((new Date().getSeconds()<10)?("0"+new Date().getSeconds()): new Date().getSeconds());
    var dia, mes, ano, hora, min, seg;
    dia = new Date().getDate();
    mes = new Date().getMonth();
    ano = new Date().getFullYear();
    hora = new Date().getHours();
    min = new Date().getMinutes();
    seg = new Date().getSeconds();
    var hora_inicio = (( ano <10)?"0"+ ano : ano ) + "-" + (( (mes+1) <10)?"0"+ (mes+1) : (mes+1)) + "-" + (( dia <10)?"0"+ dia : dia ) + "T" + (( hora <10)?"0"+ hora : hora ) + ":" + (( min <10)?"0"+ min : min ) + ":" + (( seg <10)?"0"+ seg : seg ) + "Z";
    //var hora_inicio = new Date().getDate() + "/" + (new Date().getMonth()+1) + "/" + new Date().getFullYear() + " " + new Date().getHours() + ":" + new Date().getMinutes() + ":" + new Date().getSeconds();

    //variáveis para controle de tempo

    var tempo = 0;      // Variável que armazena o tempo de execução da atividade
    var tempoInterval;  // Interval que altera o valor da variável tempo a cada segundo

    var dicaInterval;   // Interval que anima a dica
    var dicaTimeout;    // Timeout para aparecer a dica

    var mouseInterval;    // Intervalo em que o mouse é captado
    var mousePos = "";    // Vetor de plavras com as posições do Mouse

    var mousex; // Variáveis que guardarão as posições
    var mousey; // do mouse a cada movimento

    //Essa função retorna a quando o mouse é pressionado
    //*Deve ser copiada para os outros jogos
    $("#budy").mousedown(function(){
        cliqueErrado++;        //não são pressionamentos errados. são todos os pressionamentos.
        posErrado += mousex+","+mousey+";";        //pega a posição de onde o mouse foi pressionado
        ////console.log("Pressionado.");
        //var posPress;

    });

    //*essa função provavelmente está em outra linha nos outros jogos. Seria interessante colocar ela mais para cima.
    $("#budy").mousemove(function(event) {  // Atualiza mousex
        mousex = event.pageX;             // e mousey toda vez
        mousey = event.pageY;             // que o mouse se movimenta
        //event.clientX/Y é intrinsceco do JS
        ////console.log("baseSize: ",baseSize,"\nCP:",CP,"\n(",mousex,",",mousey,")", "\nValid:","\n(",validXPosition(vet[0][3]),",",validYPosition(vet[0][3]),")");
    });


    //FadeIn e FadeOut das imagens
    $("#dicaPisca1").fadeOut(0);        //fadeOut(x) esconder a imagem em x ms
    $("#dicaPisca2").fadeOut(0);
    $("#dicaPisca3").fadeOut(0);

    //dar fadeout nas estrelas
    for(i=1; i<=12;i++){
        $("#estrela"+i).fadeOut(0);
    }

    //dar fadeout nas fogos
    for(i=1; i<=8;i++){
        $("#fogos"+i).fadeOut(0);
    }

    for(i=0; i<numObjtos; i++){
        vet[i][0].fadeOut(0);
    }

    fundo.fadeOut(0);
    parabens.fadeOut(0);    // Some com o parabéns
    dica.fadeOut(0);        // Some com a dica

    
    
    var k = 500;
    for(i = 0; i<numObjtos; i++) {
         vet[i][0].fadeIn(k);        // Traz as imagens com fadeIn(x) em x ms
      k += 500; }
    fundo.fadeIn(2000);

    
    //Definindo funções de audio
    var sound = null;    // Cria um novo elemento de som no documento. esse audio é o tipo
    if(tipoSom == 1){   //palmas
	    sound = new AudioController(  '/audio/audio.mp3');         // Seta o source do elemento de som para o arquivo audio.mp3
    }
    else if(tipoSom == 2){  //plimplimplim
        sound = new AudioController(  '/audio/plim.mp3');

    }
    else if(tipoSom == 3){
        sound = new AudioController(  '/audio/somfogos.mp3');

    }

    //funções de tempo
    tempoInterval = setInterval(function() {    // Função que inicia o tempoInterval
        tempo++;                                // com a função de incrementar
    }, 1000);
    // a variável tempo a cada segundo
    mouseInterval = setInterval(function() {
        if(mousex!=null)
            mousePos += mousex + "," + mousey + ";";        //captação do mouse
    }, 100);                    //mousePos += "[" + mousex + "," + mousey + "];";
    //}, 500);

    // Essas são as funções que vão tratar
    // do DRAG AND DROP das imagens

    //  [ID][0-imagem, 1-Xinicial, 2-Yinicial, 3-larguraX, 4-larguraY,
    //      5-state, 6-tipoDica, 7-tempoDica, 8-interval, 9-target]

    //imagem 1
    vet[0][0].draggable({        //evento relacionado a imagem 1.
        start: function() {        //quando incia o evento, ativa uma função
            i=0;                //ID do vet referente a imagem
            if (vet[i][5] == 0){        //se estado é zero, a imagem está na pos inicial
                vet[i][5] = 1;            //muda de estado para 1 (aderida ao mouse)
                //vet[i][0].css({left: validXPosition(vet[i][3]), top: validYPosition(vet[i][4]), zIndex: 1});    //muda a posição da imagem de acordo com o mouse
                vet[i][0].css({zIndex: 1});
                vet[1][0].css({zIndex: 0});
                vet[2][0].css({zIndex: 0});
                cliqueCerto++;                                //acerto
                mouseDrag += "("+mousex+","+mousey+";";        //pego a posição que iniciou o drag    
                dicaTimeout = setTimeout(function() {        //chamar a dica depois do tempo
                    mostraDica(i, vet[i][6], (vet[i][9].offset().left - 100), (vet[i][9].offset().top + 60))
                }, vet[i][7]);
            }
        },
        stop: function() {        //quando termina o evento, ativa outra função
            i=0;
            mouseDrag += mousex+","+mousey+")";        //pega posição do mouse quando termina o drag
            if (mousex > vet[i][9].offset().left && mousex < (vet[i][9].offset().left + vet[i][3]) && mousey > vet[i][9].offset().top && mousey < (vet[i][9].offset().top + vet[i][4])){
				$("#container").append(vet[i][0]);        
                //Testa se o mouse (com a imagem) está dentro do target
                vet[i][0].css({left: vet[i][9].offset().left, top: vet[i][9].offset().top, zIndex: 1});    //mudo a posição da imagem para a posição do target
                vet[i][5] = 2;                    //estado da imagem passa a ser 2 indicando que foi completa
                vet[i][9].css({opacity: 0.1});
				vet[i][0].draggable('disable');        //desativo o evento drag. não pode mais ser pega
                cliqueCerto++;                    //mais um acerto.
                endgame();                        //chama função final, testando se o jogo está pronto
            }
            else         //se a imagem não estiver no target, ela volta pra posição inicial
            {
                vet[i][0].animate({left: vet[i][1], top: vet[i][2]}, 500);        //evento que anima a imagem, movendo-a
                vet[i][5] = 0;        //volta o estado pra 0.
            }
            removeDica();            //para a dica quando desativa o drag
        },
        containment: "#container", scroll: false        //impede que aparece scroll na tela
    });

    //imagem 2
    vet[1][0].draggable({
        start: function() {
            i=1;
            if (vet[i][5] == 0){
                vet[i][5] = 1;
                //vet[i][0].css({left: validXPosition(vet[i][3]), top: validYPosition(vet[i][4]), zIndex: 1});
                vet[i][0].css({zIndex: 1});
                vet[0][0].css({zIndex: 0});
                vet[2][0].css({zIndex: 0});
                cliqueCerto++;
                mouseDrag += "("+mousex+","+mousey+";";
                dicaTimeout = setTimeout(function() {
                    mostraDica(i, vet[i][6], (vet[i][9].offset().left - 100), (vet[i][9].offset().top + 60))
                }, vet[i][7]);
            }
        },
        stop: function() {
            i=1;
            mouseDrag += mousex+","+mousey+");";
            if (mousex > vet[i][9].offset().left && mousex < (vet[i][9].offset().left + vet[i][3]) && mousey > vet[i][9].offset().top && mousey < (vet[i][9].offset().top + vet[i][4])){
				$("#container").append(vet[i][0]);
                vet[i][0].css({left: vet[i][9].position().left, top: vet[i][9].position().top, zIndex: 1});
                vet[i][5] = 2;
				vet[i][9].css({opacity: 0.1});
                vet[i][0].draggable('disable');
                cliqueCerto++;
                endgame();
            }
            else 
            {
                vet[i][0].animate({left: vet[i][1], top: vet[i][2]}, 500);
                vet[i][5] = 0;
            }
            removeDica();
        },
        containment: "#container", scroll: false
    });

    //imagem 3
    vet[2][0].draggable({
        start: function() {
            i=2;
            if (vet[i][5] == 0){
                vet[i][5] = 1;
                //vet[i][0].css({left: validXPosition(vet[i][3]), top: validYPosition(vet[i][4]), zIndex: 1});
                vet[i][0].css({zIndex: 1});
                vet[1][0].css({zIndex: 0});
                vet[0][0].css({zIndex: 0});
                cliqueCerto++;
                mouseDrag += "("+mousex+","+mousey+";";
                dicaTimeout = setTimeout(function() {
                    mostraDica(i, vet[i][6], (vet[i][9].offset().left - 100), (vet[i][9].offset().top + 60))
                }, vet[i][7]);
            }
        },
        stop: function() {
            i=2;
            mouseDrag += mousex+","+mousey+");";
            if (mousex > vet[i][9].offset().left && mousex < (vet[i][9].offset().left + vet[i][3]) && mousey > vet[i][9].offset().top && mousey < (vet[i][9].offset().top + vet[i][4])){
				$("#container").append(vet[i][0]);
                vet[i][0].css({left: vet[i][9].position().left, top: vet[i][9].position().top, zIndex: 1});
                vet[i][5] = 2;
				vet[i][9].css({opacity: 0.1});
                vet[i][0].draggable('disable');
                cliqueCerto++;
                endgame();
            }
            else 
            {
                vet[i][0].animate({left: vet[i][1], top: vet[i][2]}, 500);
                vet[i][5] = 0;
            }
            removeDica();
        },
        containment: "#container", scroll: false
    });


    // Essas são as funções que tratam
    // da ADERÊNCIA da imagem ao mouse no click
    //  [ID][0-imagem, 1-Xinicial, 2-Yinicial, 3-larguraX, 4-larguraY,
    //   5-state, 6-tipoDica, 7-tempoDica, 8-interval, 9-target]
    //imagem 1
    vet[0][0].click(function() {        //evento relacionado a imagem 1. 
        i = 0;                        
        if (vet[i][5] == 0){            //se a imagem estiver na pos inical
            cliqueCerto++;                //acerto
            mouseClique += "("+mousex+","+mousey+";";        //grava a posicao do acerto
            $("body").append(vet[i][0]);
				vet[i][8] = setInterval(function() {            //é um intervalo que redefine a posição da imagem a cada 10 ms com a posição do mouse
                vet[i][0].css({left: validXPosition(vet[i][3]), top: validYPosition(vet[i][4]), zIndex: 1});
                vet[1][0].css({zIndex: 0});
                vet[2][0].css({zIndex: 0});
                vet[i][5] = 1;                        //estado 1: aderio
                vet[i][0].draggable('disable');        //desativa  drag
            }, 10);
            dicaTimeout = setTimeout(function() {        //chama a dica
                mostraDica(i, vet[i][6], (vet[i][9].offset().left - 100), (vet[i][9].offset().top + 60))
            }, vet[i][7]);
        } else if (vet[i][5] == 1) {                //se a aimgem estiver aderida
            if (mousex > vet[i][1] && mousex < (vet[i][1]+vet[i][3]) && mousey > vet[i][2] && mousey < (vet[i][2]+vet[i][4])) {
				$("#container").append(vet[i][0]);            //testa se a posição do mouse está dentro da posição incial (desistencia)
                clearInterval(vet[i][8]);    //limpo intervalo para liberar a imagem do mouse
                vet[i][0].css({left: vet[i][1], top: vet[i][2]});        //volta para posição inicial
                vet[i][5] = 0;                                            //estado zero
                vet[i][0].draggable('enable');                            //reativa drag
                mouseClique += mousex+","+mousey+");";                    //pego a posição do clique
                removeDica();                                            //remove dica
            } else if (mousex > vet[i][9].offset().left && mousex < (vet[i][9].offset().left + vet[i][3]) && mousey > vet[i][9].offset().top && mousey < (vet[i][9].offset().top + vet[i][4])) {                //testar se a imagem está no target
                vet[i][0].css({left: vet[i][9].offset().left, top: vet[i][9].offset().top});
                vet[i][5] = 2;
				vet[i][9].css({opacity: 0.1});
                clearInterval(vet[i][8]);
                vet[i][0].draggable('disable');
                removeDica();
                cliqueCerto++;
                mouseClique += mousex+","+mousey+");";
                endgame();
            }
        }

    });

    //imagem 2
    vet[1][0].click(function() {
        i = 1;
        if (vet[i][5] == 0){
            cliqueCerto++;
            mouseClique += "("+mousex+","+mousey+";";
            $("body").append(vet[i][0]);
				vet[i][8] = setInterval(function() {
                vet[i][0].css({left: validXPosition(vet[i][3]), top: validYPosition(vet[i][4]), zIndex: 1});
                vet[0][0].css({zIndex: 0});
                vet[2][0].css({zIndex: 0});
                vet[i][5] = 1;
                vet[i][0].draggable('disable');
            }, 10);
            dicaTimeout = setTimeout(function() {
                mostraDica(i, vet[i][6], (vet[i][9].offset().left - 100), (vet[i][9].offset().top + 60))
            }, vet[i][7]);
        } else if (vet[i][5] == 1) {
            if (mousex > vet[i][1] && mousex < (vet[i][1]+vet[i][3]) && mousey > vet[i][2] && mousey < (vet[i][2]+vet[i][4])) {
				$("#container").append(vet[i][0]);
                clearInterval(vet[i][8]);
                vet[i][0].css({left: vet[i][1], top: vet[i][2]});
                vet[i][5] = 0;
                vet[i][0].draggable('enable');
                mouseClique += mousex+","+mousey+");";
                removeDica();
            } else if (mousex > vet[i][9].offset().left && mousex < (vet[i][9].offset().left + vet[i][3]) && mousey > vet[i][9].offset().top && mousey < (vet[i][9].offset().top + vet[i][4])) {
				$("#container").append(vet[i][0]);
                vet[i][0].css({left: vet[i][9].position().left, top: vet[i][9].position().top});
                vet[i][5] = 2;
				vet[i][9].css({opacity: 0.1});
                clearInterval(vet[i][8]);
                vet[i][0].draggable('disable');
                removeDica();
                cliqueCerto++;
                mouseClique += mousex+","+mousey+");";
                endgame();
            }
        }
    });

    //imagem 3
    vet[2][0].click(function() {
        i = 2;
        if (vet[i][5] == 0){
            cliqueCerto++;
            mouseClique += "("+mousex+","+mousey+";";
            $("body").append(vet[i][0]);
				vet[i][8] = setInterval(function() {
                vet[i][0].css({left: validXPosition(vet[i][3]), top: validYPosition(vet[i][4]), zIndex: 1});
                vet[0][0].css({zIndex: 0});
                vet[1][0].css({zIndex: 0});
                vet[i][5] = 1;
                vet[i][0].draggable('disable');
            }, 10);
            dicaTimeout = setTimeout(function() {
                mostraDica(i, vet[i][6], (vet[i][9].offset().left - 100), (vet[i][9].offset().top + 60))
            }, vet[i][7]);
        } else if (vet[i][5] == 1) {
            if (mousex > vet[i][1] && mousex < (vet[i][1]+vet[i][3]) && mousey > vet[i][2] && mousey < (vet[i][2]+vet[i][4])) {
				$("#container").append(vet[i][0]);
                clearInterval(vet[i][8]);
                vet[i][0].css({left: vet[i][1], top: vet[i][2]});
                vet[i][5] = 0;
                vet[i][0].draggable('enable');
                mouseClique += mousex+","+mousey+");";
                removeDica();
            } else if (mousex > vet[i][9].offset().left && mousex < (vet[i][9].offset().left + vet[i][3]) && mousey > vet[i][9].offset().top && mousey < (vet[i][9].offset().top + vet[i][4])) {
				$("#container").append(vet[i][0]);
                vet[i][0].css({left: vet[i][9].position().left, top: vet[i][9].position().top, zIndex: 1});
                vet[i][5] = 2;
				vet[i][9].css({opacity: 0.1});
                clearInterval(vet[i][8]);
                vet[i][0].draggable('disable');
                removeDica();
                cliqueCerto++;
                mouseClique += mousex+","+mousey+");";
                endgame();
            }
        }
    });

    /*--------------------------------------------------------------------*/
    //funções que retornam uma posição dentro da 'caixa branca'. ou o container do jogo

    function validXPosition(x) {               // Função que               //etro x
        var primaryPosition = mousex - (x/2);   // retorna                  //é a largura da imagem
        if (primaryPosition < 0)                // uma posição              //necessária para que
            return 0;                       // válida em                    //posição do mouse
        else if (primaryPosition > (820 + $("#container").offset().left-(x/2)))     // X, dentro                    //fique no meio
            return (820 + $("#container").offset().left-(x/2));                     // dos limites da               //imagem clicada
        return primaryPosition;             // área da atividade
    }

    function validYPosition(y) {                // Função que
        var primaryPosition = mousey - (y/2);   // retorna
        if (primaryPosition < 0)                // uma posição
            return 0;                       // válida em
        else if (primaryPosition > (720 + $("#container").offset().top-(y/2)))     // Y, dentro
            return (720 + $("#container").offset().top-(y/2));                     // dos limites da
        return primaryPosition;             // área da atividade
    }


    function mostraDica(i, td, x, y){        //função que determina qual dica será usada
        //rd = rand(1,2)    
        if(td == "Seta"){            //caso seja dica seta
            dica.css({left: x, top: y, zIndex: 2});        //muda a posição da imagem SETA.
            dica.fadeIn(500);                            //faz aparecer a seta
            dicaInterval = setInterval(animaDica, 1810);    //intervalo de tempo que chama a animação
        }else{                        //caso a dica seja pisca
            //vet[i][0].css({left: x, top: y, zIndex: 2});    
            //mostraDicaPisca(i);
            dicaInterval = setInterval( function() {mostraDicaPisca(i);}, 1810);    //intervalo de tempo que chama a animação
        }
        quantDicasUsadas++;        //conta quantas dicas foram usadas
    }

    function mostraDicaPisca(i){            //anima a opacidade do target da imagem
        vet[i][9].animate({opacity: '0.1'}, "slow");
        vet[i][9].animate({opacity: '1.0'}, "slow");
    }


    function animaDica() {                        //anima a seta para direita e esquerda (ou alto e baixo)
        dica.animate({left: (dica.offset().left)}); 
        dica.animate({left: (dica.offset().left - 30)});
        dica.animate({left: (dica.offset().left)});
        //dica.animate({width: 'toggle'}, 500);
    }

    function removeDica() {
        clearInterval(dicaInterval);            //limpa os intervalo para parar de chamar a função
        clearTimeout(dicaTimeout);
        dica.fadeOut(200);                        //esconder a seta
    }

    function animateParabens() {
        parabens.animate({top: 180}, 200);  // Essa
        parabens.animate({top: 280}, 200);  // função
        parabens.animate({top: 200}, 200);  // anima
        parabens.animate({top: 260}, 200);  // o parabéns
        parabens.animate({top: 240}, 200);  // com
        parabens.animate({top: 260}, 200);  // pulinhos
    }

    function animaEstrela(){                //anima a estrela
        i=1;                                //inicia iterador
        var parabensInterval = setInterval(function(){        //a cada 600ms, aparece uma nome imagem.
            $('#estrela'+i).fadeIn(590); 
            i++;
        }, 600);
        setTimeout(function() {
            clearInterval(parabensInterval);                //limpa para desaparecer tudo
        }, 7200);
    }

    function estrelaMusical(){                        //mesma função acima, porém sicronizada com som PLIMPLIM
        i=1;
        var parabensInterval = setInterval(function(){
            if(i == 2 || i == 6 || i == 10){
                sound.play();  
            }
            $('#estrela'+i).fadeIn(590);
            i++;
        }, 600);
        setTimeout(function() {
            clearInterval(parabensInterval);
        }, 7200);
    }

    function animaFogos(){            //anima os fogos aparecendo cada imagem de uma vez. 
        var i=1;
        var parabensInterval = setInterval(function(){
            if(i>1){
                $('#fogos'+(i-1)).fadeOut(100);
            }
            $('#fogos'+i).fadeIn(610); 
            i++;
        }, 600);
        setTimeout(function() {
            clearInterval(parabensInterval);
        }, 7200);
    }


    //FUNÇÃO DE FIM DE JOGOS
    function endgame() {
        if (vet[0][5] == 2 && vet[1][5] == 2 && vet[2][5] == 2) {           // Se as três imagens já estiverem nos alvos, dipara

            finalizado = 1;                                //faz nada

            for(i=0; i<numObjtos; i++){                    //desaparece com as imagens
                vet[i][0].fadeOut(200);    
                vet[i][9].fadeOut(200);
            }

            fundo.fadeOut(200);                            //desaparece com o fundo (caso tenha)
            removeDica();                                //remove a dica (mais por garantia, caso bugue nas funções)

            //substituir daqui                toca o som
            if(tipoSom == 0){
                //sem som
            }
            else if (tipoSom == 1){
				sound.play();       // Toca o som das palmas

            }
            else if ((tipoSom == 2 || tipoSom ==3) && comemoracao != 3){        // plimplim sem estrela
                i = 0;
                var parabensInterval = setInterval(function(){
                   sound.play();
                }, 500);
                setTimeout(function() {
                    clearInterval(parabensInterval);
                }, 7200);
            }

            if(comemoracao == 0){            //parabens estrela, fogos
                //sem comemoração
            }
            //parabens animado
            else if(comemoracao == 1){
                parabens.fadeIn(500);                               // Trás à tona o parabéns com fadeIn
                animateParabens();                                  // Anima o parabéns uma vez
                var parabensInterval = setInterval(function() {     // Cria um interval para
                    animateParabens();                              // animar o parabéns a cada 1200ms
                }, 1200);                                           // NOTA: 1200ms não é aleatório, é o tamanho do ciclo da animação
                setTimeout(function() {                             // Cria um timeout para limpar
                    clearInterval(parabensInterval);                // o parabensInterval depois de 7200ms
                }, 7200);                                           // (6 ciclos), se não ele ia quicar para sempre
            }
            //parabens parado
            else if(comemoracao == 2){ //parabéns parado
                parabens.fadeIn(500);
            }
            //estrela
            else if(comemoracao == 3 && tipoSom != 2){    //estrela sem plimplim 
                animaEstrela();
            }
            //fogos
            else if(comemoracao == 4){                    //fogos
                animaFogos();
            }

            //caso especial da estrela e o plim estarem ativados
            if(tipoSom == 2 && comemoracao == 3){
                estrelaMusical();
            }

            setTimeout(function(){                //volta para o menu
                goBack("/Jogos/menu/?" + urlGlobal);

            },8500);

            console.log(new Date())
            //até aqui    

            //console.log('dicas: '+quantDicasUsadas);

            //chama a função de gravação de dados
            ajaxSave();
            //salvaDados();
        }
    }

    function salvaDados(){
        var jogador;
        do{
            jogador = prompt("Qual seu Nome?");
        }while(jogador == null || jogador == "");
        var data = new Date().getFullYear() + "-" + (((new Date().getMonth()+1)<10)?("0"+(new Date().getMonth()+1)):((new Date().getMonth()+1))) + "-" + ((new Date().getDate()<10)?("0"+new Date().getDate()):(new Date().getDate()));
        var arquivo = "dados.properties"
        //var hora_fim = new Date().getHours() + ":" + new Date().getMinutes() + ":" + new Date().getSeconds();
        var hora_fim = ((new Date().getHours()<10)?("0"+new Date().getHours()): new Date().getHours())+ ":" + ((new Date().getMinutes()<10)?("0"+new Date().getMinutes()): new Date().getMinutes()) + ":" + ((new Date().getSeconds()<10)?("0"+new Date().getSeconds()): new Date().getSeconds());
        var blob = new Blob(["dados.server.jogador = "+jogador+"\ndados.server.data = "+data+"\ndados.server.inicio = "+hora_inicio+"\ndados.server.termino = "+hora_fim+"\ndados.server.dicas = "+quantDicasUsadas+"\ndados.server.mousepos = "+mousePos], {type: "text/plain;charset=utf-8"});
        saveAs(blob, arquivo);
    }


    //desabilita botão direito
    function disableselect(e){   
        return false   
    }   

    function reEnable(){   
        return true   
    }   

    //if IE4+   
    document.onselectstart=new Function ("return false")   
    document.oncontextmenu=new Function ("return false")   

    //if NS6   
    if (window.sidebar){   
        document.onmousedown=disableselect   
        document.onclick=reEnable   
    }

    function ajaxSave() {
        dia = new Date().getDate();
        mes = new Date().getMonth();
        ano = new Date().getFullYear();
        hora = new Date().getHours();
        min = new Date().getMinutes();
        seg = new Date().getSeconds();

        var hora_fim = (( ano <10)?"0"+ ano : ano ) + "-" + (( (mes+1) <10)?"0"+ (mes+1) : (mes+1)) + "-" + (( dia <10)?"0"+ dia : dia ) + "T" + (( hora <10)?"0"+ hora : hora ) + ":" + (( min <10)?"0"+ min : min ) + ":" + (( seg <10)?"0"+ seg : seg ) + "Z";

        //console.log(hora_inicio);
        //console.log(hora_fim);

        var requisicaoDaqui = "RESULTADO";
        var idAtividade = 1;    //alterar esse id para o número do jogo
        var idSessao = parseInt(getQueryVariable("campoEscondidoIDSessao"));
        var jsonObj = JSON.stringify({
		"idatividade": {"id": idAtividade},
		"idsessao": {"id": idSessao},
		"cliquecerto": cliqueCerto,
		"mousepos": mousePos,
		"mouseclique": mouseClique,
		"mousedrag": mouseDrag,
		"poserrado": posErrado,
		"cliqueerrado": cliqueErrado,
		"dicas": quantDicasUsadas,
		"horainicio": hora_inicio,
		"horafim": hora_fim
	});

	saveController(jsonObj, "/Jogos/Resultados", 'post');
        
    }
});
