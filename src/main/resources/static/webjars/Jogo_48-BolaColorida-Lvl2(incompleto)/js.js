...
/*    // aqui estou pegando os valores passados na url de cada jogo
    ...
    var tipodeDica ="Seta";                                  //"Seta" ou "Pisca"
    var DicaTempoSouN = "True";                       //true ou false
    var tempodeDica = 2*1000;        //numero
    var DicaCaminhoSouN = false;                   //true ou false
    var comemoracao = 2;                    //retorna 0-sem comemoração, 1-parabens animado, 2-parabens parado
    var tipoSom = 0;                                 //retorna 0-sem som, 1- palmas
    var musicaFundo = 0;                    // retorn 0-sem musica 

    var numObjtos = 8; //quantidade de figuras
    var diferente = 2; // quantas imgs diferente terão de ser selecionadas
    var targets = new Array(2); // [0 - numero da diferente, 1 - state]
    
    
    
    var aux, j, jj; // auxiliares para randomizar
    jj=0;
...
//  [ID][0-imagem, 1-Xinicial, 2-Yinicial, 3-larguraX, 4-larguraY,
    ...
    vet = new Array(
        new Array($("#img"+sortido[0]), 50, 70, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[0])),
        new Array($("#img"+sortido[1]), 240, 70, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[1])), 
        new Array($("#img"+sortido[2]), 430, 70, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[2])),  
        new Array($("#img"+sortido[3]), 620, 70, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[3])),
        new Array($("#img"+sortido[4]), 50, 240, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[4])),  
        new Array($("#img"+sortido[5]), 240, 240, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[5])),
        new Array($("#img"+sortido[6]), 430, 240, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[6])),
        new Array($("#img"+sortido[7]), 620, 240, 150, 150, 0, tipodeDica, tempodeDica, 0, $("#target"+sortido[7]))
    );
...
//imagem 1
    vet[0][0].draggable({
        ...
        stop: function() {
            ...
            mouseDrag += mousex+","+mousey+")";
                //
            //se sortido[0] == 4 || sortido[0] == 7 && tgaux4 == 0 || tgaux7 ==0
    ...
    vet[7][0].draggable({
        ...
        stop: function() {
            ...
            if (mousex > vet[i][9].offset().left && mousex < (vet[i][9].offset().left + 800) && mousey > vet[i][9].offset().top && mousey < (vet[i][9].offset().top + 150)){
				...
				vet[i][5] = 2;
                vet[i][0].draggable('disable');
                //Variáveis novas que devem ser copiadas para os novs jogos
                cliqueCerto++;
                //
                endgame();
            }
...
function animaEstrela(){
    ...
    function endgame() {
        ...
        if(auxcont == 2){// Nesse jogo, apenas duas entrarão no target
    ...
    function ajaxSave() {
        ...
        var idAtividade = 48;    //alterar esse id para o número do jogo
